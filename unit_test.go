...
func TestSend(t *testing.T) {
	...
	info := new(Info)
	info.SetFrom(args[4])              //${{ secrets.FROM }}
	tos := strings.Split(args[5], ",") //${{ secrets.TO }}
	info.SetTo(tos)
	info.SetSubject(args[6]) //保活
	info.SetText(args[7])    //发自github工作流
	//info.SetImage(args[9])
	info.SetHost(QQ.SMTP)
	info.SetPort(QQ.SMTPProt)
	info.SetUsername(args[5]) //${{ secrets.FROM }}
	info.SetPassword(args[8]) //${{ secrets.PASSWORD }}
	t.Logf("%+v\n", info)
	status := info.Send()
	t.Log(status)
}
